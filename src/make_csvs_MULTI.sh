...
csv_file="${out_dir}"/STATS_MULTI/seedfractions.csv
> "${csv_file}"
...
for source in ${source_regions} ; do
	for LR in L R ; do
		...
		for target in ${target_regions} ; do
			csv_line="${source}_${LR}_to_TARGETS_${LR},${source}_${LR},${nvox},${target}_${LR}"
			bit="$(csv_line.py ${rois_dwi_dir}/${source}_${LR}.nii.gz seeds_to_${target}_${LR}.nii.gz seedtotal.nii.gz \
